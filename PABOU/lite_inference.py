...
def bench_lite_one_model(st, TFlite_file, dataset, h5_size, coral=False, nb = 10, func = None, metadata = None):
    ...
    if coral:
        try:
            delegate = tf.lite.experimental.load_delegate(EDGETPU_SHARED_LIB) #  2 bong on windows
            interpreter = tf.lite.Interpreter(model_path = TFlite_file, experimental_delegates=[delegate]) 
            print('LITE: interpreter uses CORAL delegate. loaded')
        except Exception as e:
        ...
        print('LITE: interpreter does not use CORAL. ') 
    ...
    input_details= interpreter.get_input_details() # input_details is a list of dictionaries
    print('LITE: input details: ', input_details[0]['shape'], input_details[0]['dtype'])
        
    if metadata != None:
        print('LITE: TFlite model name: %s. description: %s' %(metadata['name'] , metadata['description']))

    for a,_ in dataset.take(1): # get batch size
        dataset_batch_size = a.shape[0]
        break
    print('LITE: benchmark dataset: %d elements, of batch size: %d'  %(len(dataset), dataset_batch_size))
    ...
    for images,labels in dataset.take(nb):  # returns nb batch of eager tensors . each for returns TensorShape([1, 96, 96, 3])
        ...
        for i in range(images.shape[0]): #  all images in one batch, can be 32 or 1
    ...
    # create table just for this test  basename 'lili_fp32.tflite'
    pt = PrettyTable()
    ...
    test_name = test_name.split('_')[1:]  # remove app name
